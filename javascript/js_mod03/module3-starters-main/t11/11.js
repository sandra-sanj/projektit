...
for (const item of picArray) {
  ...
  for (const newElement of [title, figure, paragraph]) {
  ...
  function openArticle(dialog) {
    const imgLarge = dialog.getElementsByTagName('img')[0];
    imgLarge.src = item.image['large'];
    imgLarge.alt = item.title;
  }
  ...
  newArticle.addEventListener('click', () => {
    ...
    openArticle(dialogElement);
  });
  
  // close modal
  const spanElement = dialogElement.getElementsByTagName('span')[0];
